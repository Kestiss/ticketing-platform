...
class AdmissionService(
    ...
    fun createScanner(command: CreateScannerDeviceCommand): ScannerBootstrapResult {
        ...
        val device = ScannerDevice(UUID.randomUUID(), command.organizationId, displayName, SecureToken.hash(rawSecret), ScannerDeviceStatus.ACTIVE, Instant.now(clock), null)
        scanners.create(device)
        scanners.assignToEvent(device.id, command.eventId, Instant.now(clock))
    ...
    fun validate(command: ValidateAdmissionCommand): AdmissionDecision {
        ...
        val entitlement = jdbc.query(
            """SELECT entitlement.id, entitlement.status AS entitlement_status, credential.status AS credential_status,
                 credential.version FROM ticket_entitlement entitlement JOIN ticket_credential credential
                 ON credential.ticket_entitlement_id = entitlement.id
                 WHERE credential.id = :credentialId AND credential.version = :version
                   AND entitlement.event_id = :eventId""",
    ...
    private fun constantTimeEquals(left: String, right: String): Boolean = java.security.MessageDigest.isEqual(left.toByteArray(), right.toByteArray())
...
data class ValidateAdmissionCommand(val scannerDeviceId: UUID, val scannerSecret: String, val eventId: UUID, val presentationClaim: String)
